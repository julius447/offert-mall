...
| `offert.referens` | string | **yes** | header pill | `Offert #2026-0187` |
...
`bokare.namn` fills all five places from one key. Do not try to set them individually.
...
Note the quote page uses `giltig_till_**kort**` and the declined page
`giltig_till_**lang**`. Two different formats, two keys, on purpose.
...
### 5.5 Behaviour you must decide (not designed yet)
...
| Expired quote | The page has a complete expired state, but **no production trigger.** Send `offert.ar_utgangen: true` (or add `class="is-expired"` to `<body>` server-side). Without it an expired quote is fully acceptable. |
...
- [ ] Search the rendered HTML for `[name]`, `[Bokare]`, `[GAP]`, `EXEMPEL`, `Bokarens text`. **Expect zero hits.** This is the single most important check: an unmapped field ships a placeholder to a paying customer.
